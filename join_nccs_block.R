...
block_paths <- list.files(path, full.names = TRUE)
...
nccs_data_sf <- st_as_sf(nccs_data, coords = c("Longitude", "Latitude"), crs = 4326) %>% 
  select(id, STATE, Longitude, Latitude) %>%
...
full_joined_dat <- list.files("intermediate", pattern = "joined_block_", full.names = TRUE) %>% 
...
nccs_dat_full <- left_join(nccs_data, full_joined_dat, by = c("id", "STATE")) %>% 
